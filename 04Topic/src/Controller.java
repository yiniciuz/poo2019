...
class Topic {
    ...
    public Topic(int capacidade, int quantPreferenciais){
        poltronas = new ArrayList<Passageiro>();
        for(int i = 0; i < capacidade; i++){
            poltronas.add(null);
        }
        this.quantPreferenciais = quantPreferenciais;
    }

    public String toString(){
        String saida = "[ ";
        for(int i = 0; i < poltronas.size(); i++){
            if(i < quantPreferenciais)
                saida += "@ ";
            else
                saida += "= ";
    ...
    public void inserir(Passageiro pass){
        if (pass.preferencial()){
            ...
            if(!inserir(this.quantPreferenciais, this.poltronas.size(), pass));{
                inserir(0, this.quantPreferenciais, pass);
            }
        }
    }
